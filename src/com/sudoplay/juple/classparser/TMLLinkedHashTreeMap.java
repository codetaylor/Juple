 ...
 */
public final class TMLLinkedHashTreeMap<K, V> extends AbstractMap<K, V>
  ...
  public TMLLinkedHashTreeMap(Comparator<? super K> comparator) {
    ...
    if (nearest != null) {
      ...
      while (true) {
        comparison = (comparableKey != null) ? comparableKey
            .compareTo(nearest.key) : comparator.compare(key, nearest.key);
   ...
   * 
   * <p>
   * This method uses the comparator for key equality rather than {@code equals}
   * . If this map's comparator isn't consistent with equals (such as
   * {@code String.CASE_INSENSITIVE_ORDER}), then {@code remove()} and
   * {@code contains()} will violate the collections API.
   */
  ...
  void removeInternal(Node<K, V> node, boolean unlink) {
    ...
    if (left != null && right != null) {
      ...
      /*
       * To remove a node with both left and right subtrees, move an adjacent
       * node from one of those subtrees into this node's place.
       * 
       * Removing the adjacent node may change this node's subtrees. This node
       * may no longer have two subtrees once the adjacent node is gone!
       */

      Node<K, V> adjacent = (left.height > right.height) ? left.last() : right
          .first();
  ...
  private void rotateLeft(Node<K, V> root) {
    ...
    pivot.height = Math.max(root.height, pivotRight != null ? pivotRight.height
        : 0) + 1;
  }
  ...
  private void rotateRight(Node<K, V> root) {
    ...
    pivot.height = Math.max(root.height, pivotLeft != null ? pivotLeft.height
        : 0) + 1;
  }
   ...
   * the height of <i>N.left</i> and <i>N.right</i> is different by at most 1.
   * It accomplishes this by omitting deepest-level leaf nodes when building
   * trees whose size isn't a power of 2 minus 1.
   * 
   * <p>
  ...
  static class AvlBuilder<K, V> {
    ...
    void reset(int targetSize) {
      // compute the target tree size. This is a power of 2 minus one, like 15
      // or 31.
    ...
    void add(Node<K, V> node) {
       ...
       * less than a multiple of 8. For example at 8 we may combine subtrees (A
       * B C) and (E F G) with D as the root to form ((A B C) D (E F G)).
       * 
  ...
  class EntrySet extends AbstractSet<Entry<K, V>> {
    ...
    @Override
    public void clear() {
      TMLLinkedHashTreeMap.this.clear();
    }
  }
