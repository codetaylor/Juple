 ...
 */
public final class TMLSqlDateTypeAdapter extends TMLTypeAdapter<java.sql.Date> {

  private final DateFormat enUsFormat;
  private final DateFormat localFormat;
  private final DateFormat iso8601Format;

  public TMLSqlDateTypeAdapter() {
    this(new SimpleDateFormat("MMM d, yyyy", Locale.US), new SimpleDateFormat(
        "MMM d, yyyy"));
  }

  public TMLSqlDateTypeAdapter(String datePattern) {
    this(new SimpleDateFormat(datePattern, Locale.US), new SimpleDateFormat(
        datePattern));
  }

  public TMLSqlDateTypeAdapter(int style) {
    this(DateFormat.getDateInstance(style, Locale.US), DateFormat
        .getDateInstance(style));
  }

  public TMLSqlDateTypeAdapter(int dateStyle, int timeStyle) {
    this(DateFormat.getDateTimeInstance(dateStyle, timeStyle, Locale.US),
        DateFormat.getDateTimeInstance(dateStyle, timeStyle));
  }

  public TMLSqlDateTypeAdapter(DateFormat enUsFormat, DateFormat localFormat) {
    this.enUsFormat = enUsFormat;
    this.localFormat = localFormat;
    this.iso8601Format = new SimpleDateFormat("MMM d, yyyy", Locale.US);
    this.iso8601Format.setTimeZone(TimeZone.getTimeZone("UTC"));
  }

  @Override
  public java.sql.Date read(TMLReader in) throws IOException {
  ...
  @Override
  public synchronized void write(TMLWriter out, java.sql.Date value)
      throws IOException {
    if (value == null) {
      out.nullValue();
      return;
